...
export const data = new SlashCommandBuilder()
  .setName("forcecreateplugin")
  .setDescription("Force create a marketplace plugin")
  ...
  .addStringOption((option) =>
    option
      .setName("type")
      .setDescription(
        "The plugin type. Either page (run on a specific website) OR SW (uses workerware, see our docs)"
      )
      .setRequired(true)
      .addChoices(
        { name: "Page", value: "plugin-page" },
        { name: "SW", value: "plugin-sw" }
      )
  );
...
export async function execute(interaction: any) {
  ...
  if (interaction.member.roles.cache.has("1303567697610539049")) {
    ...
    const exampleEmbed = new EmbedBuilder()
      ...
      .setAuthor({ name: interaction.options.getString("description") })
